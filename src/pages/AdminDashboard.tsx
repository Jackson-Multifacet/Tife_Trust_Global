...
import { Users, Activity, DollarSign, UserPlus, ArrowUpRight, Settings, ShieldAlert } from "lucide-react"
...
export default function AdminDashboard() {
  ...
  return (
    <motion.div 
      ...
      animate="show"
    >
      <motion.div variants={itemVariants} className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-primary/10 via-primary/5 to-transparent p-6 rounded-xl border">
        <div>
      ...
      <motion.div variants={itemVariants} className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="hover:shadow-md transition-all duration-200 hover:-translate-y-1">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Active sessions right now</p>
          </CardContent>
        </Card>
      </motion.div>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-start gap-4 rounded-lg border p-4 bg-green-500/10 border-green-500/20">
              ...
              <div className="flex-1 space-y-1">
                <p className="text-sm font-medium leading-none text-green-700 dark:text-green-400">System Update Complete</p>
                <p className="text-sm text-green-600/80 dark:text-green-400/80">All services running normally.</p>
              </div>
            ...
            <div className="flex items-start gap-4 rounded-lg border p-4 bg-yellow-500/10 border-yellow-500/20">
              ...
              <div className="flex-1 space-y-1">
                <p className="text-sm font-medium leading-none text-yellow-700 dark:text-yellow-400">High Traffic Alert</p>
                <p className="text-sm text-yellow-600/80 dark:text-yellow-400/80">Unusual spike in portal logins.</p>
              </div>
            ...
            <div className="flex items-start gap-4 rounded-lg border p-4 bg-destructive/10 border-destructive/20">
              ...
              <div className="flex-1 space-y-1">
                <p className="text-sm font-medium leading-none text-destructive">Failed Login Attempts</p>
                <p className="text-sm text-destructive/80">Multiple failed logins from IP 192.168.1.45.</p>
              </div>
      ...
      <motion.div variants={itemVariants} className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
        <Card className="col-span-4">
          ...
          <CardContent>
            <div className="space-y-6">
              {[
                { name: "Sarah Jenkins", email: "sarah.j@tifetrust.com", cases: 142, trend: "+12%" },
                { name: "Michael Chen", email: "m.chen@tifetrust.com", cases: 128, trend: "+8%" },
                { name: "Aisha Patel", email: "a.patel@tifetrust.com", cases: 114, trend: "+15%" },
                { name: "David Kim", email: "david.k@tifetrust.com", cases: 98, trend: "-2%" },
                { name: "Emma Wilson", email: "emma.w@tifetrust.com", cases: 85, trend: "+5%" },
              ].map((staff, i) => (
                <div key={i} className="flex items-center justify-between p-2 hover:bg-muted/50 rounded-lg transition-colors">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      ...
                      <p className={`text-xs ${staff.trend.startsWith('+') ? 'text-green-500' : 'text-red-500'}`}>
                        {staff.trend}
                      </p>
                    </div>
                  </div>
        ...
        <Card className="col-span-3">
          ...
          <CardContent className="grid gap-4">
            <Button variant="outline" className="justify-start h-14" render={<Link to="/portal/admin/users" />}>
              <UserPlus className="mr-4 h-5 w-5 text-muted-foreground" />
              <div className="text-left">
                <div className="font-semibold">Add New Staff</div>
                <div className="text-xs text-muted-foreground font-normal">Create a new staff account</div>
              </div>
            </Button>
            <Button variant="outline" className="justify-start h-14" render={<Link to="/portal/admin/settings" />}>
              <Settings className="mr-4 h-5 w-5 text-muted-foreground" />
              <div className="text-left">
                <div className="font-semibold">System Configuration</div>
                <div className="text-xs text-muted-foreground font-normal">Update global settings</div>
              </div>
            </Button>
            <Button variant="outline" className="justify-start h-14">
              <Activity className="mr-4 h-5 w-5 text-muted-foreground" />
              <div className="text-left">
                <div className="font-semibold">Generate Audit Report</div>
                <div className="text-xs text-muted-foreground font-normal">Download system logs</div>
              </div>
            </Button>
          </CardContent>
        </Card>
