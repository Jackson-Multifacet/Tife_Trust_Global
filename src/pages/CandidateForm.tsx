...
export default function CandidateForm() {
  ...
  const handleInputChange = (
    ...
    } else if (name === "phone" || name === "nokPhone") {
      value = sanitizePhone(value);
    } else if (["nin", "bvn"].includes(name)) {
      value = value.replace(/[^0-9]/g, "").substring(0, 11);
    } else if (name !== "amountNeeded" && name !== "tenor") {
  ...
  const handleFileChange = async (
    ...
    try {
      ...
      const downloadURL = await uploadFile(
        file,
        `applications/${Date.now()}_${file.name.substring(0, 10)}`,
      );
      setFileUrls((prev: any) => ({ ...prev, [field]: downloadURL }));
  ...
  const validateStep = () => {
    ...
    if (currentStep === 5) {
      ...
      const missingFiles = currentRequired.filter(
        (field: string) => !fileUrls[field],
      );
    ...
    const missingFields = currentRequired.filter(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const submissionData = {
        ...
        submittedByEmail: formData.email, // For Firestore rule validation
      };
      ...
      const missingUrls = requiredUrls.filter(
        (url) => !submissionData[url as keyof typeof submissionData],
      );
      ...
      setError(
        "Failed to submit application. Please ensure all required fields are filled correctly.",
      );
      ...
      <div className="container max-w-2xl py-20 mx-auto px-4">
        ...
        >
          ...
          <p className="text-muted-foreground text-lg max-w-md mx-auto">
            Thank you for choosing Tife Trust Global. Your application is being
            processed and our team will contact you within 24-48 hours.
          </p>
          <div className="pt-8">
            <Button
              onClick={() => navigate("/")}
        ...
        <div className="flex justify-between items-center mb-8 relative">
          ...
          {STEPS.map((step, i) => (
            ...
            >
              ...
              <span
                className={`text-[10px] uppercase tracking-wider font-bold mt-2 hidden md:block ${
        ...
        <Card className="shadow-xl border-primary/5">
              ...
              {currentStep === 1 && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      onValueChange={(v) =>
                        handleSelectChange("maritalStatus", v)
                      }
              ...
              {currentStep === 4 && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      onValueChange={(v) => handleSelectChange("nokGender", v)}
                      defaultValue={formData.nokGender}
                    >
                      <SelectTrigger>
                        <SelectValue placeholder="Select gender" />
                      </SelectTrigger>
                  ...
                  <div className="md:col-span-2 space-y-2">
                    <Label htmlFor="nokResidentialAddress">
                      Residential Address
                    </Label>
                    <Input
                      id="nokResidentialAddress"
                      name="nokResidentialAddress"
                      value={formData.nokResidentialAddress}
                      onChange={handleInputChange}
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <Input
                      id="nokNearestBusStop"
                      name="nokNearestBusStop"
                      value={formData.nokNearestBusStop}
                      onChange={handleInputChange}
                    />
                  </div>
                  <div className="space-y-2">
              ...
              {currentStep === 5 && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {[
                    {
                      id: "validId",
                      label: "Valid ID (With names as on BVN)",
                      desc: "National ID, Passport, or Driver's License",
                    },
                    {
                      id: "utilityBill",
                      label: "Utility Bill (Current)",
                      desc: "Electricity, Water, or Waste bill",
                    },
                    {
                      id: "passportPhoto",
                      label: "Passport Photo",
                      desc: "Clear, recent passport-sized photograph",
                    },
                    {
                      id: "signature",
                      label: "Signature",
                      desc: "Signed on plain white paper",
                    },
                  ].map((doc) => (
                    <div
                      key={doc.id}
                      className="space-y-4 p-6 rounded-2xl border-2 border-dashed border-muted hover:border-primary/50 transition-colors bg-muted/5"
                    >
                      ...
                      {fileUrls[doc.id] && (
                        <p className="text-[10px] text-green-600 font-medium flex items-center gap-1">
                          <Check className="h-3 w-3" /> File uploaded
                          successfully
                        </p>
                      )}
                    </div>
          ...
          <CardFooter className="flex justify-between border-t bg-muted/5 p-6">
            ...
            {currentStep === STEPS.length - 1 ? (
              ...
              <Button onClick={nextStep} className="gap-2 px-8 rounded-full">
